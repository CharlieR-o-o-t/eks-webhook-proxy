...
version::ldflags() {
    ...
    add_ldflag "BUILD_DATE" "$(date ${SOURCE_DATE_EPOCH:+"--date=@${SOURCE_DATE_EPOCH}"} -u +'%Y-%m-%dT%H:%M:%SZ')"
    add_ldflag "GIT_COMMIT" "${GIT_COMMIT}"
    add_ldflag "GIT_TREE_STATE" "${GIT_TREE_STATE}"
    add_ldflag "GIT_VERSION" "${GIT_VERSION}"
    add_ldflag "GIT_RELEASE_COMMIT" "${GIT_RELEASE_COMMIT}"
